...
class CdnCleaner {
  ...
  Future<Null> purgePackages(Iterable<Package> packages) {
    ...
    var relativePaths = packages.map((package) {
      return new Directory(package.outputDir(config)).listSync(recursive: true).where((f) => f is File).map((file) {
  ...
  Future<Null> _purgeCache(Map<String, Object> payload) async {
    var request = new Request(
        "DELETE",
        Uri.parse("https://api.cloudflare.com/client/v4/zones/${config.cloudflareZone}/purge_cache"));
