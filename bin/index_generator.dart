...
main(List<String> args) async {
  try {
    ...
    if (argsResults["help"]) {
      print(
          "Generates index pages (like /index.html, /failed/index.html, etc) and uploads them to GCS in an infinite loop.\n");
      print(parser.usage);
      return;
    }
    logging.initialize();
    ...
    while (true) {
      ...
      _logger.info(
          "Last number of successfully generated packages is $previousSuccessfulPackagesLength");
      _logger.info(
          "Current number of successfully generated packages is ${successPackages.length}");
      if (previousSuccessfulPackagesLength != successPackages.length) {
        _logger
            .info("There are new packages available, regenerating the index");
      ...
      _logger.info(
          "Last number of erroneously generated packages is $previousErroredPackagesLength");
      _logger.info(
          "Current number of erroneously generated packages is ${errorPackages.length}");
      if (previousErroredPackagesLength != errorPackages.length) {
        _logger.info(
            "There are new failed packages available, regenerating the errors index");
      ...
      if (previousSuccessfulPackagesLength != successPackages.length ||
        ...
        _logger
            .info("There are new packages available, regenerating the history");
        var sortedPackages = datastoreRetriever.allPackages.toList()
          ..sort((a, b) => b.updatedAt.compareTo(a.updatedAt));
        await indexGenerator.generateHistory(
            sortedPackages
                .getRange(0, min(sortedPackages.length, 100))
                .toList(),
            successPackages);
        _logger.info("Done");
